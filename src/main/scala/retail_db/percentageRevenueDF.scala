...
object percentageRevenueDF {
  def main(args: Array[String]): Unit = {
    val props = ConfigFactory.load()
    val conf = new SparkConf().setMaster(props.getConfig(args(0)).getString("executionMode")).setAppName("revenuepercentage")
    ...
    val rawdata = lastdata.select("category_name","department_name","order_item_subtotal")

    val result = rawdata.groupBy("category_name","department_name").agg(avg("order_item_subtotal").alias("avgRevenue"))
